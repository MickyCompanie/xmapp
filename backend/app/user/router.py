...
@user_router.get("/", 
                 dependencies=[Depends(authorized_role(UserRole.ADMIN, UserRole.SANTA))], 
...
@user_router.put("/{id}", dependencies=[Depends(authorized_role(UserRole.ADMIN, UserRole.SANTA))], response_model=UserRead, status_code=status.HTTP_200_OK)
...
@user_router.post("/reactivate/{id}", dependencies=[Depends(authorized_role(UserRole.ADMIN, UserRole.SANTA))], response_model=UserRead, status_code=status.HTTP_200_OK)
def reactivate_user(id: int, user_in: UserDelete, db: Session = Depends(get_db)):
