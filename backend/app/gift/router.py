...
@gift_router.get('/', dependencies=[Depends(must_be_authenticated)], response_model=list[GiftRead], status_code=status.HTTP_200_OK)
def get_all_gifts(db: Session = Depends(get_db)):
    return gift_service.get_all_gifts(db)
